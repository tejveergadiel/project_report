...
def initialize_session_state():
    ...
    if 'initialized' not in st.session_state:
        ...
        # COS data variables - Dynamic tower storage with auto-detection
        st.session_state.cos_towers = {}  # Dynamic tower data: {tower_name: {'df': dataframe, 'tname': tower_display_name}}
        st.session_state.detected_towers = []  # List of all detected tower names
...
# Auto-detect and store tower data dynamically
def store_tower_data(tower_name, dataframe):
    """
    Automatically detect and store any tower data dynamically.
    This function supports any current or future tower names without code changes.
    
    Args:
        tower_name: The display name of the tower (e.g., "Tower 4(A)", "Tower 5", "Tower 8")
        dataframe: The pandas DataFrame containing tower data
    """
    if dataframe is None or dataframe.empty:
        safe_log(f"Skipping empty dataframe for {tower_name}", "warning")
        return
    
    # Extract tower identifier from tower_name (e.g., "4A" from "Tower 4(A)")
    import re
    # Try multiple patterns to extract tower identifier
    patterns = [
        r'Tower\s*(\d+[A-Z]?)',  # Matches "Tower 4A", "Tower 4(A)", "Tower 5", etc.
        r'Tower\s*(\d+)\(([A-Z])\)',  # Matches "Tower 4(A)" format
        r'([A-Za-z0-9\-_]+)',  # Fallback: any alphanumeric identifier
    ]
    
    tower_id = None
    for pattern in patterns:
        match = re.search(pattern, tower_name)
        if match:
            if len(match.groups()) > 1:
                # For "Tower 4(A)" format, combine them
                tower_id = f"{match.group(1)}{match.group(2)}"
            else:
                tower_id = match.group(1)
            break
    
    if not tower_id:
        tower_id = tower_name  # Fallback to full tower_name
    
    # Store tower data dynamically
    if 'cos_towers' not in st.session_state:
        st.session_state.cos_towers = {}
    if 'detected_towers' not in st.session_state:
        st.session_state.detected_towers = []
    
    # Store the dataframe and tower name
    st.session_state.cos_towers[tower_id] = {
        'df': dataframe,
        'tname': tower_name,
        'rows': len(dataframe)
    }
    
    # Add to detected towers list if not already there
    if tower_id not in st.session_state.detected_towers:
        st.session_state.detected_towers.append(tower_id)
        st.session_state.detected_towers.sort()  # Sort for consistency
    
    safe_log(f"Tower {tower_id} ({tower_name}) auto-detected and stored: {len(dataframe)} rows")
    return tower_id

# Helper function to retrieve tower data
def get_tower_data(tower_id_or_name):
    """
    Retrieve tower data by tower ID or tower name.
    
    Args:
        tower_id_or_name: Tower ID (e.g., "4A") or tower name (e.g., "Tower 4(A)")
        
    Returns:
        tuple: (dataframe, tower_name, tower_id) or (None, None, None) if not found
    """
    if 'cos_towers' not in st.session_state or not st.session_state.cos_towers:
        return None, None, None
    
    # Try direct ID lookup first
    if tower_id_or_name in st.session_state.cos_towers:
        tower_info = st.session_state.cos_towers[tower_id_or_name]
        return tower_info['df'], tower_info['tname'], tower_id_or_name
    
    # Try searching by tower name
    for tower_id, tower_info in st.session_state.cos_towers.items():
        if tower_info['tname'] == tower_id_or_name:
            return tower_info['df'], tower_info['tname'], tower_id
...
def get_all_towers():
    """
    Get all detected towers with their data.
    
    Returns:
        dict: {tower_id: {'df': dataframe, 'tname': tower_name, 'rows': count}}
    """
    if 'cos_towers' not in st.session_state:
        return {}
    return st.session_state.cos_towers
            ...
            if files:
                ...
                for selected_file in files:
                    try:
                        ...
                        if len(result) == 2:  # Handle Tower 4 split or other multi-part files
                            (df_first, tname_first), (df_second, tname_second) = result
                            
                            # Process first dataframe - auto-detect tower
                            if df_first is not None and not df_first.empty:
                                store_tower_data(tname_first, df_first)
                            
                            # Process second dataframe - auto-detect tower
                            if df_second is not None and not df_second.empty:
                                store_tower_data(tname_second, df_second)
                            ...
                            if df_first is not None and not df_first.empty and tname_first == "Slab Cycle":
                                ...
                                # Auto-detect any other single tower file
                                store_tower_data(tname_first, df_first)
                        else:
...
async def GetWorkspaceID():
    ...
    try:
        ...
        safe_log(f"Error fetching Workspace ID: {str(e)}", "error")
        raise
    finally:
        session.close()
...
def process_data(df, activity_df, location_df, dataset_name, use_module_hierarchy_for_finishing=False):
    ...
    asite_activities = [
        ...
        "Saucer drain/Paver block", "Kerb Stone", "Concreting", "De-shuttering"
    ]
...
def AnalyzeStatusManually(email=None, password=None):
    ...
    # Validate COS data - NOW FULLY DYNAMIC (auto-detects any tower)
    st.write("### Validating COS Data...")
    
    # Ensure cos_towers dictionary exists
    if 'cos_towers' not in st.session_state:
        st.session_state.cos_towers = {}
    if 'detected_towers' not in st.session_state:
        st.session_state.detected_towers = []
    
    # Display all detected towers dynamically
    if st.session_state.detected_towers:
        st.write(f"**Auto-Detected Towers:** {', '.join(st.session_state.detected_towers)}")
        for tower_id in st.session_state.detected_towers:
            tower_info = st.session_state.cos_towers.get(tower_id, {})
            tower_name = tower_info.get('tname', tower_id)
            rows = tower_info.get('rows', 0)
            if rows > 0:
                st.write(f"  • {tower_name}: {rows} rows ✓")
            else:
                st.write(f"  • {tower_name}: EMPTY ✗")
    else:
        st.write("❌ No towers detected. Please load COS files first.")
    ...
    # Process COS data - NOW FULLY DYNAMIC (any towers auto-detected)
    ...
    # Process COS data for each dynamically detected tower
    if 'cos_towers' not in st.session_state or not st.session_state.cos_towers:
        st.warning("No towers detected. Please load COS files first.")
    else:
        for tower_id, tower_info in st.session_state.cos_towers.items():
            tower_data = tower_info.get('df')
            tower_name = tower_info.get('tname', tower_id)
            
            first_fix_counts[tower_name] = {}
            st.write(f"Processing {tower_name} (ID: {tower_id})")
            ...
            if tower_data is not None and isinstance(tower_data, pd.DataFrame) and not tower_data.empty:
                ...
                if missing_columns:
                    st.warning(f"Missing columns in {tower_name}: {missing_columns}")
                    ...
                    for activity in activities_list:
                        ...
                        if activity in ["UP-First Fix", "CP-First Fix"]:
                            first_fix_counts[tower_name][activity] = 0
                else:
                    ...
                    logger.info(f"Processing {tower_name}: {len(tower_data_filtered)} rows with actual finish dates")
                    st.write(f"✓ Processing {tower_name}: {len(tower_data_filtered)} rows with actual finish dates")
                    ...
                    for activity in activities_list:
                        ...
                        logger.info(f"{tower_name} - {activity}: {count} counts")
                        if activity in ["UP-First Fix", "CP-First Fix"]:
                            first_fix_counts[tower_name][activity] = count
            else:
                st.write(f"No data available for {tower_name}, setting all counts to 0")
                for activity in activities_list:
                    cos_data.append({
                    "Tower": tname,
                    "Activity Name": activity,
    ...
    logger.info(f"Slab data for checklist summary generation: {slab_data}")

    # Generate structured summary with local functions
    ...
    try:
        ...
        if ai_response and ai_response != combined_data:
            ...
            logger.info("Checklist summary generated successfully")
        else:
            st.warning("Summary generation failed, using fallback data")
            fallback_data = {
...
# Initialize session state variables - NOW USING DYNAMIC TOWER SYSTEM
if 'cos_towers' not in st.session_state:
    st.session_state.cos_towers = {}
if 'detected_towers' not in st.session_state:
    st.session_state.detected_towers = []
...
def process_file(file_stream, filename):
    try:
        ...
        if is_slab_cycle:
            ...
            # Handle Tower 5, Tower 6, or Tower 7 Finishing Tracker files
            tower_num = None
...
def generatePrompt(combined_data, slab):
    try:
        cos_df = combined_data["COS"] if isinstance(combined_data.get("COS"), pd.DataFrame) else pd.DataFrame()
        asite_df = combined_data["Asite"] if isinstance(combined_data.get("Asite"), pd.DataFrame) else pd.DataFrame()
        ...
        def parse_slab_data(raw_slab):
            if isinstance(raw_slab, pd.DataFrame):
                return raw_slab
            if isinstance(raw_slab, str):
                try:
                    parsed = json.loads(raw_slab)
                    ...
                    return pd.DataFrame()
            if isinstance(raw_slab, list):
                return pd.DataFrame(raw_slab)
            if isinstance(raw_slab, dict):
                return pd.DataFrame([raw_slab])
            return pd.DataFrame()

        def normalize_counts(df, source_name):
            if df.empty:
                return df
            required_columns = {"Tower", "Activity Name", "Count"}
            if not required_columns.issubset(df.columns):
                logger.warning(f"Missing required columns for {source_name}: {required_columns - set(df.columns)}")
                return pd.DataFrame(columns=["Tower", "Activity Name", "Count"])

            normalized_df = df[["Tower", "Activity Name", "Count"]].copy()
            normalized_df["Tower"] = normalized_df["Tower"].fillna("Unknown Tower").astype(str).str.strip()
            normalized_df["Activity Name"] = normalized_df["Activity Name"].fillna("").astype(str).str.strip()
            normalized_df["Count"] = pd.to_numeric(normalized_df["Count"], errors="coerce").fillna(0).astype(int)
            normalized_df = normalized_df[normalized_df["Activity Name"] != ""]
            return normalized_df

        def build_source_output(df, source_name):
            normalized_df = normalize_counts(df, source_name)
            normalized_df = normalized_df[
                ~normalized_df["Tower"].isin(["Tower 4(A)", "Tower 4(B)", "T4A", "T4B"])
            ].copy()
            configured_towers = ["Tower 5", "Tower 6", "Tower 7"]
            discovered_towers = []
            if not normalized_df.empty:
                discovered_towers = sorted(tower for tower in normalized_df["Tower"].unique() if tower)

            tower_order = []
            for tower in configured_towers + discovered_towers:
                if tower not in tower_order:
                    tower_order.append(tower)

            towers_output = []
            for tower in tower_order:
                tower_df = normalized_df[normalized_df["Tower"] == tower] if not normalized_df.empty else pd.DataFrame()
                grouped_counts = {}
                if not tower_df.empty:
                    grouped_counts = tower_df.groupby("Activity Name")["Count"].sum().to_dict()

                category_output = []
                for category_name, activities in categories[source_name].items():
                    category_output.append({
                        "Category": category_name,
                        "Activities": [
                            {
                                "Activity Name": activity_name,
                                "Total": int(grouped_counts.get(activity_name, 0))
                            }
        ...
        slab_df = parse_slab_data(slab)
        slab_totals = {}
        if not slab_df.empty:
            parsed_slab = slab_df.copy()
            if "Completed" not in parsed_slab.columns and "Green (1)" in parsed_slab.columns:
                parsed_slab["Completed"] = parsed_slab["Green (1)"]
            if {"Tower", "Completed"}.issubset(parsed_slab.columns):
                parsed_slab = parsed_slab[["Tower", "Completed"]].copy()
            else:
                parsed_slab = pd.DataFrame()

        if not slab_df.empty and not parsed_slab.empty:
            parsed_slab["Tower"] = parsed_slab["Tower"].astype(str).str.strip()
            parsed_slab["Tower"] = (
                parsed_slab["Tower"]
                .str.replace("TOWER ", "T", regex=False)
                .str.replace("Tower ", "T", regex=False)
            )
            parsed_slab["Completed"] = pd.to_numeric(parsed_slab["Completed"], errors="coerce").fillna(0).astype(int)
            slab_totals = dict(zip(parsed_slab["Tower"], parsed_slab["Completed"]))

        structured_data = {
            "COS": build_source_output(cos_df, "COS"),
            "Asite": build_source_output(asite_df, "Asite"),
            "Slab": slab_totals
        }

        logger.info("Generated checklist summary using deterministic local functions")
        return json.dumps(structured_data, indent=2)

    except Exception as e:
        logger.error(f"Error generating checklist summary: {str(e)}", exc_info=True)
        fallback_data = {
            "COS": combined_data["COS"].to_dict("records") if isinstance(combined_data.get("COS"), pd.DataFrame) else [],
            "Asite": combined_data["Asite"].to_dict("records") if isinstance(combined_data.get("Asite"), pd.DataFrame) else [],
            "Slab": {}
        }
    ...
    try:
        ...
        if not slab_df.empty:
            ...
            for _, row in slab_df.iterrows():
                ...
                if tower in ['T4', 'T4A', 'T4B', 'Tower 4', 'Tower 4(A)', 'Tower 4(B)']:
                    continue
        ...
        categories = {
            ...
            "Asite": {
                ...
                "Interior Finishing Works": [
                    "Waterproofing - Sunken", "Wall Tile", "Floor Tile", 
...
def generate_consolidated_Checklist_excel(ai_data):
    try:
        ...
        # **NEW FIX: Ensure T6 has all Civil Works activities**
        logger.info("=== ADDING MISSING T6 CIVIL WORKS ACTIVITIES ===")
        
        # Find T6 Concreting count
        t6_concreting_count = 0
        for row in consolidated_rows:
            if row["Tower"] == "T6" and row["Category"] == "Civil Works" and row["Activity Name"] == "Concreting":
                t6_concreting_count = row["Completed Work*(Count of Flat)"]
                logger.info(f"Found T6 Concreting count: {t6_concreting_count}")
                break
        
        # Ensure all Civil Works activities exist for T6
        if t6_concreting_count > 0:
            civil_works_activities = ["Shuttering", "Reinforcement", "Concreting", "De-shuttering"]
            existing_t6_activities = {row["Activity Name"] for row in consolidated_rows 
                                     if row["Tower"] == "T6" and row["Category"] == "Civil Works"}
        ...
        # Expand slab-derived Concreting counts into the full Civil Works set for any tower.
        logger.info("=== ADDING MISSING CIVIL WORKS ACTIVITIES FROM SLAB COUNTS ===")
        civil_works_activities = ["Shuttering", "Reinforcement", "Concreting", "De-shuttering"]
        civil_concreting_counts = {}

        for row in consolidated_rows:
            if row["Category"] == "Civil Works" and row["Activity Name"] == "Concreting":
                civil_concreting_counts[row["Tower"]] = row["Completed Work*(Count of Flat)"]

        for tower_name, concreting_count in civil_concreting_counts.items():
            if concreting_count <= 0:
                continue

            existing_civil_activities = {
                row["Activity Name"]
                for row in consolidated_rows
                if row["Tower"] == tower_name and row["Category"] == "Civil Works"
            }

            for activity_name in civil_works_activities:
                if activity_name in existing_civil_activities:
                    continue

                asite_key = (tower_name, activity_name, "Civil Works")
                asite_count_for_activity = asite_data_dict.get(asite_key, {"count": 0})["count"]
                new_row = {
                    "Tower": tower_name,
                    "Category": "Civil Works",
                    "Activity Name": activity_name,
                    "Completed Work*(Count of Flat)": concreting_count,
                    "In Progress Work*(Count of Flat)": 0,
                    "Closed checklist against completed work": asite_count_for_activity,
                    "Open/Missing check list": max(0, concreting_count - asite_count_for_activity)
                }
                consolidated_rows.append(new_row)
                logger.info(
                    f"Added missing {tower_name} Civil Works activity: {activity_name} "
                    f"with COS count {concreting_count} and Asite count {asite_count_for_activity}"
                )
                st.write(
                    f"Added {tower_name} {activity_name} with COS={concreting_count}, "
                    f"Asite={asite_count_for_activity}"
                )
        ...
        # **FINAL T6 CHECK IN DATAFRAME**
        logger.info("=== FINAL T6 CHECK IN DATAFRAME ===")
        t6_df = df[df['Tower'] == 'T6']
        logger.info(f"T6 rows in DataFrame: {len(t6_df)}")
        if not t6_df.empty:
            logger.info("T6 activities in DataFrame:")
            st.write("✓ T6 activities in final DataFrame:")
            for _, row in t6_df.iterrows():
                logger.info(f"  {row['Category']} - {row['Activity Name']}: COS={row['Completed Work*(Count of Flat)']}, Asite={row['Closed checklist against completed work']}, Open={row['Open/Missing check list']}")
                st.write(f"  {row['Category']} - {row['Activity Name']}: COS={row['Completed Work*(Count of Flat)']}, Open={row['Open/Missing check list']}")
        else:
            logger.error("❌ T6 has NO rows in DataFrame!")
            st.error("❌ T6 has NO rows in DataFrame!")
        ...
        # Keep only the required Veridia towers in the final export.
        desired_towers = ['T5', 'T6', 'T7']
        logger.info(f"Towers before filtering: {tower_data['Tower'].unique().tolist()}")
        tower_data = tower_data[tower_data['Tower'].isin(desired_towers)].copy()
        logger.info(f"Towers after filtering: {tower_data['Tower'].unique().tolist()}")
        
        if 'T6' not in tower_data['Tower'].values:
            logger.error("❌ T6 was filtered out during tower filtering!")
            st.error("❌ T6 was filtered out during tower filtering!")
        else:
            logger.info("✓ T6 survived filtering")
            st.write("✓ T6 survived filtering")
        ...
        for tower, tower_group in grouped_by_tower:
            ...
            for category, cat_group in grouped_by_category:
                ...
                worksheet1.cell(row=current_row, column=6).value = f"{tower} {current_month} Checklist Status - {category}"
        ...
        if not external_data.empty:
            ...
            current_month = datetime.now().strftime("%B")
            worksheet1.cell(row=current_row, column=6).value = f"External Development {current_month} Checklist Status"
            worksheet1.cell(row=current_row, column=6).font = category_font
            current_row += 1
        ...
        worksheet2 = workbook.create_sheet(title=f"Checklist {datetime.now().strftime('%B')}")
        current_row = 1

        worksheet2.cell(row=current_row, column=1).value = f"Checklist: {datetime.now().strftime('%B')}"
        ...
        logger.info(f"Towers in summary after filtering: {df_for_summary['Tower'].unique().tolist()}")
        if 'T6' in df_for_summary['Tower'].values:
            logger.info("✓ T6 is in summary data")
            st.write("✓ T6 is in summary data for Sheet 2")
        else:
            logger.error("❌ T6 is NOT in summary data")
            st.error("❌ T6 is NOT in summary data for Sheet 2")
        ...
        # Check if T6 is in summary
        if any('06' in site for site in summary_data.keys()):
            logger.info("✓ Tower 6 found in summary data")
            st.write("✓ Tower 6 found in summary data")
        else:
            logger.warning("❌ Tower 6 NOT in summary data")
            st.warning("❌ Tower 6 NOT in summary data")
        ...
        if not summary_data:
            ...
            for site_name, counts in sorted(summary_data.items()):
                ...
                if total_count > 0:
                    ...
                    if '06' in site_name:
                        logger.info(f"✓ Added Tower 6 to Sheet 2: {site_name}")
                        st.write(f"✓ Added Tower 6 to Sheet 2: {site_name}")
            ...
            if files:
                ...
                for selected_file in files:
                    try:
                        ...
                        if len(result) == 2:
                            # Handle Tower 4 split or other multi-part files
                            (df_first, tname_first), (df_second, tname_second) = result
